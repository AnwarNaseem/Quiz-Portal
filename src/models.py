...
from marshmallow.schema import Schema, ValidationError
...
"""
username = 'root'
password = 'S0c1h0e1m0a1'
database_name = 'quiz_app'
application.config['SQLALCHEMY_DATABASE_URI'] = f"mysql://{root}:{S0c1h0e1m0a1}@localhost/{QuizMaster}"
...
class QuizQuestions(db.Model):
        ...
        question_id = db.Column(db.String(200),db.ForeignKey('quiz_master.question'))
        is_active = db.Column(db.Integer, default=1)
        created_ts = db.Column(db.DateTime, default = datetime.utcnow)
        updated_ts = db.Column(db.DateTime)
...
class QuizInstance(db.Model):
        ...
        quiz_id = db.Column(db.String(200),db.ForeignKey('quiz_master.id'))
        user_id = db.Column(db.String(200),db.ForeignKey('user_master.id'))
        score_achieved = db.Column(db.Integer(100),default=0)
        is_submitted = db.Column(db.Integer(100),default=0)
        is_active = db.Column(db.Integer, default=1)
        created_ts = db.Column(db.DateTime, default = datetime.utcnow)
        updated_ts = db.Column(db.DateTime)
...
class UserResponses(db.Model):
        ...
        response = db.Column(db.Integer(100),default=0)
        is_active = db.Column(db.Integer, default=1)
        created_ts = db.Column(db.DateTime, default = datetime.utcnow)
        updated_ts = db.Column(db.DateTime)
